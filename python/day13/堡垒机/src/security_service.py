...
DATABASE = Config(config_path, section='database')
...
engine = create_engine("mysql+pymysql模块://{user}:@{host}:{port}/{database}"
                       .format(user=DB_USER, host=DB_HOST, port=DB_PORT, database=DB), max_overflow=5)
...
class UserProfile(Base):
    ...
    # 如果是一个人只能在一个组下
	group_id = Column(Integer,ForeignKeyey('group.id'))
...
# 输入用户名和密码：
#
obj = session.query(UserProfile).filter(username=DB_USER, password=DB_PASSWORD).first()
...
class Session:
    ...
    session = None
	def __init__():
		engine = create_engine("mysql+pymysql模块://root:123@127.0.0.1:3306/s13", max_overflow=5)
