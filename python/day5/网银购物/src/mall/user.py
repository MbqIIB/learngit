...
def registered(user, password, bank_card):
    '''
    用于用户注册
    :param user: 用户名
    :param password:  密码
    ...
    if not os.path.exists(USER_DB):
        json.dump({'admin': {'lock': 0, 'password': 'admin'}}, open(USER_DB, 'w'))
    with open(USER_DB, 'r') as load_user_file:
    ...
    if user in user_file_dict.keys():
        ...
        with open(USER_DB, 'w') as dump_user_file:
...
def login(user, password):
    ...
    :return:
    """
    with open(USER_DB, 'r') as user_file:
            ...
            if user_dict[user]['lock'] == 1:   # 判断是否是锁定用户
                ...
                while flag:
                    ...
                    if user_counter >2:  # 输入错误2次以上
                        ...
                        with open(USER_DB, 'w') as user_file:
